...
export default class Search extends Component {
        ...
        fetch( url, { credentials: 'include' } ).then( ( res ) => res.json() )
            .then( ( layers ) => {
                let layerTypeNames = layers.objects.map( ( layer ) => {
                    return { value: layer.typename, label: layer
                            .title }
                } )
    ...
    render() {
        return (
            <Select name="form-field-name" placeholder="Search Layers" value={this.state.inputValue} options={this.state.layerTypeNames != undefined
