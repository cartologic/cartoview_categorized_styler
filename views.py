...
def save_style(request, layer_name, style_name):
    ...
    new = style is None
    xml = request.body
    gs_catalog.create_style(style_name, xml, True,workspace=settings.DEFAULT_WORKSPACE)
    if new:
        style = gs_catalog.get_style(style_name,workspace=settings.DEFAULT_WORKSPACE)
